    ...
    <ul class="menu-inner py-1">
     ...
     <!-- <li class="menu-item">
      ...
      @endif
                        ...
                        <section class="py-3 py-md-5">
                            <div class="row justify-content-center">
                                <div style="margin-top: -40px;" class="col-12 col-lg-9 col-xl-8 col-xxl-7">
                                    ...
                                    <h5><small> Invoice created by {{$invoice->user->name}} on {{$invoice->created_at->format('F l d, Y, H:i A')}} and updated by {{$invoice->user1?->name}} at {{$invoice->updated_at->format('F l d, Y, H:i A')}} </small></h5>
                                </div>
                        </section>
                    </div>
                </div>
        ...
        <!-- / Layout wrapper -->
        @if($invoice->status != 'completed' && $invoice->status != 'uncompleted' && Auth::user()->hasRole(['Invoice']))
        <div class="buy-now">
            <a style="margin-bottom: 75px;" href="/invoice/{{$invoice->id}}/edit" class="btn btn-danger btn-buy-now"> <i class="icon-base bx bx-edit-alt me-2 text-primary"></i> Edit</a>
        </div>
        @endif
        <div class="buy-now">
            <button id="print"
